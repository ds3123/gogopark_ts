...
{ /* 價格表單欄位  */ }
const Price_Form : FC<Price> = ( { register  , errors , setValue , current , species_Id, source_Create_Way, data } ) => {
    ...
    useEffect(( ) : any => {
       ...
       // # 設定 _ 初次洗澡、單次洗澡、包月洗澡、單次美容、包月美容 5 種價格 "初始值"
        if( source_Create_Way === '寵物品種' ){
           ...
           setValue( 'price_Fist_Bath' ,     first_Bath['service_price']  ) ;
           setValue( 'price_Single_Bath' ,   single_Bath['service_price']  ) ;
           setValue( 'price_Month_Bath' ,    month_Bath['service_price']  ) ;
           setValue( 'price_Single_Beauty' , single_Beauty['service_price']  ) ;
           setValue( 'price_Month_Beauty' ,  month_Beauty['service_price']  ) ;
