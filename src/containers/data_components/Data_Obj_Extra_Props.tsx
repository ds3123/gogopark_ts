...
const Get_Plan_Data = () => {
    ...
    return { month_Bath_Price , month_Beauty_Price , current_Plan_Id , current_Plan_Note }

} ;

// 取得 _ "住宿"、"安親" 相關資料
const Get_Lodge_Care_Data = () => {
...
export const Add_Data_Obj_Extra_Props = ( current : string , data : any ) => {

    // # 取得 _ 愈賦加屬性的值
    const { month_Bath_Price , month_Beauty_Price , current_Plan_Id , current_Plan_Note } = Get_Plan_Data();

    const { basicSumPrice , bathSumPrice , beautySumPrice , extraItemFee , extraBeautyFee , current_Create_Service_Type  } = Get_Service_Prices()

    const { expect_Care_End_Time , current_Lodge_Price_Sum } = Get_Lodge_Care_Data() ;

    const { service_Status , current_Q_Code } = Get_Info_Data() ;
